  ...
  <form @submit.prevent="createArrangement">
    <div class="modal-body">
      ...
      <div class="input-group mb-3">
        ...
        <input type="number" class="form-control" step="0.01" min="0.01" aria-label="Amount (to the nearest dollar)"
          v-model="editable.vasePrice" required id="vase" aria-describedby="vase-price" />
      </div>
      <div class="d-flex justify-content-between">
        <button class="btn btn-primary mb-3" type="button" @click="toggleDropdown('#budget-input')"
          aria-expanded="false">
          Budget?
        </button>
        <button class="btn btn-primary mb-3" type="button" @click="toggleDropdown('#labor-input')"
          aria-expanded="false">
          Adjust Labor?
        </button>
      </div>
      <div class="collapse" id="budget-input">
        <div class="mb-3 input-group">
          ...
          <input type="number" step="0.01" class="form-control" id="budget" placeholder="Budget..."
            v-model="editable.budget" />
        </div>
      </div>
      <div class="collapse" id="labor-input">
        <div class="d-flex justify-content-between mb-3">
          <button type="button" :class="`btn btn-${editable.labor == 0.8 ? 'success' : 'outline-dark'
    }`" @click="adjustLabor(0.8)">
            20%
          </button>
          <button type="button" :class="`btn btn-${editable.labor == 0.75 ? 'success' : 'outline-dark'
    }`" @click="adjustLabor(0.75)">
            25%
          </button>
          <button type="button" :class="`btn btn-${editable.labor == 0.7 ? 'success' : 'outline-dark'
